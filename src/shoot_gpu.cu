...
__device__ static double d_wrappedDist(double a, double b) {
    double d = a - b;
    // bring to (-pi, pi]
    d -= 2.0 * M_PI * floor((d + M_PI) / (2.0 * M_PI));
    return fabs(d);
}
    ...
    for (int s = 0; s < n_steps; ++s) {
        ...
        if (denom > 0.0) {
            ...
            if (t > 0.0 && t < 1.0) {
                ...
                double di  = d_wrappedDist(ith, target_th);
